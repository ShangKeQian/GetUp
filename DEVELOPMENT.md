...
**图标绘制：** 64×64 彩色圆 + 白色 "G" 字母，颜色随状态变化。

**右键菜单（状态感知）：**

| 状态 | 菜单内容 |
|------|----------|
| **工作中** | 标题 "GetUp · 工作中"、已工作时间、下次休息倒计时、在位检测状态、摄像头状态、暂停监控、设置、退出 |
| **无人** | 标题 "GetUp · 无人"、无人状态、摄像头待机、设置、退出 |
| **已暂停** | 标题 "GetUp · 已暂停"、监控已暂停、恢复监控、设置、退出 |
| **休眠中** | 标题 "GetUp · 休眠"、智能休眠中、摄像头已释放、唤醒、设置、退出 |

**图标绘制（v2.1.1）：** 64×64 彩色圆 + 白色图形（人形/双人/暂停/月亮），颜色随状态变化。

**新增 API（v2.1.0）：**
- `update_work_elapsed(seconds)` — 更新工作时间内部状态
- `update_camera_status(active)` — 更新摄像头状态显示
